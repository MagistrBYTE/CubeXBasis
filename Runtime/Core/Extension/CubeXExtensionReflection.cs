...
//---------------------------------------------------------------------------------------------------------------------
/** \file CubeXExtensionReflection.cs
...
namespace CubeX
{
	namespace Core
	{
		...
		public static class XExtensionReflectionType
		{
			...
			public const String COLLECTION_MODEL_1 = "CollectionModel`1";

			/// <summary>
			/// Префикс имени для модулей и сборок платформы CubeX
			/// </summary>
			public const String CUBEX_PREFIX = "CubeX";
			...
			//---------------------------------------------------------------------------------------------------------
			/// <summary>
			/// Проверка на принадлежность типа к платформе CubeX
			/// </summary>
			/// <param name="this">Тип</param>
			/// <returns>Статус проверки</returns>
			//---------------------------------------------------------------------------------------------------------
			public static Boolean IsCubeXPlatformType(this Type @this)
			{
			...
			/// <returns>Статус проверки</returns>
			//---------------------------------------------------------------------------------------------------------
			public static Boolean IsListCubeXType(this Type @this)
			{
			...
			//---------------------------------------------------------------------------------------------------------
			/// <summary>
			/// Проверка на коллекцию для хранения и управления моделями
			/// </summary>
			/// <param name="this">Тип</param>
			/// <returns>Статус проверки</returns>
			//---------------------------------------------------------------------------------------------------------
			public static Boolean IsCollectionModel(this Type @this)
			{
				Boolean status = (@this.IsGenericType && (@this.Name == COLLECTION_MODEL_1));
				if (status == false)
				{
					// Пробуем проверить базовый тип
					Type base_type = @this.BaseType;
					if (base_type != null)
					{
						status = (base_type.IsGenericType && (base_type.Name == COLLECTION_MODEL_1));
					}
					...
					if (status == false && base_type != null)
					{
						base_type = @this.BaseType.BaseType.BaseType.BaseType;
						if (base_type != null)
						{
							status = (base_type.IsGenericType && (base_type.Name == COLLECTION_MODEL_1));
						}
					...
					if (status == false && base_type != null)
					{
						base_type = @this.BaseType.BaseType.BaseType.BaseType.BaseType;
						if (base_type != null)
						{
							status = (base_type.IsGenericType && (base_type.Name == COLLECTION_MODEL_1));
						}
