...
//---------------------------------------------------------------------------------------------------------------------
/** \file CubeXUnityEditorReorderableList.cs
...
namespace CubeX
{
	namespace Core
	{
		...
		public sealed class CReorderableList : IDisposable
		{
			...
			// Работа с атрибутами
			internal CubeXReorderableAttribute mReorderAttribute;
			internal List<CubeXColumnAttribute> mColumnsAttribute;
			...
			public CReorderableList(UnityEditor.SerializedProperty property)
			{
				...
				{
					// Получаем коллекцию
					mItemsProperty = mProperty.GetCubeXInnerCollection();
					mCollection = mProperty.GetValue<IList>();
			...
			private void Init()
			{
				...
				if (mProperty != null)
				{
					mReorderAttribute = mProperty.GetAttribute<CubeXReorderableAttribute>();
					...
					{
						if(XEditorCachedData.GetAttribute<CubeXInLineAttribute>(mProperty) != null)
						{
							mIsDrawInline = true;
						}
					}
			...
			public Single GetHeight()
			{
				...
				{
					return (XEditorInspector.CONTROL_HEIGHT_SPACE + XEditorInspector.SPACE);
				}
			}
			...
			public void DrawItemDefault(UnityEditor.SerializedProperty item, Rect position, Int32 index)
			{
				...
				if (UnityEditor.EditorGUI.EndChangeCheck())
				{
					...
					{
						if (mCollection is ICubeXNotifyCollectionChanged)
						{
							item.Save();
							ICubeXNotifyCollectionChanged collection_changed = mCollection as ICubeXNotifyCollectionChanged;
			...
			public void DrawItemInLine(UnityEditor.SerializedProperty item, Rect position, Int32 index)
			{
				UnityEditor.EditorGUI.BeginChangeCheck();
				{
					...
					{
						mColumnsAttribute = new List<CubeXColumnAttribute>();
						for (Int32 i = 0; i < childs.Count; i++)
						{
							CubeXColumnAttribute column_attribute = XEditorCachedData.GetAttribute<CubeXColumnAttribute>(childs[i]);
					...
					if (index == 0)
					{
						...
						rect_header.height = XEditorInspector.CONTROL_HEIGHT_SPACE;
						for (Int32 i = 0; i < childs.Count; i++)
						{
					...
					if (index == 0)
					{
						rect_index.y -= XEditorInspector.CONTROL_HEIGHT_SPACE / 2;
					}
			...
			public void Reset(UnityEditor.SerializedProperty property)
			{
				...
				{
					// Получаем коллекцию
					mItemsProperty = property.GetCubeXInnerCollection();
			...
			private Single OnHeightItemDefault(Int32 index)
			{
				...
				else
				{
					...
					else
					{
						...
						{
							if (mItemsProperty == null) mItemsProperty = mProperty.GetCubeXInnerCollection();
							if (mItemsProperty != null)
							{
						...
						if (item != null)
						{
							if (mIsDrawInline)
							{
								Single max_height = XEditorInspector.CONTROL_HEIGHT_SPACE;
								List<UnityEditor.SerializedProperty> childs = item.GetChildrenProperties();
								...
								for (Int32 i = 0; i < childs.Count; i++)
								{
									...
									{
										max_height = height + XEditorInspector.SPACE;
									}
								}

								if (index == 0)
								{
									max_height += XEditorInspector.CONTROL_HEIGHT_SPACE;
								}
							...
							{
								Single default_height = XEditorInspector.CONTROL_HEIGHT;
								Single compute_height = UnityEditor.EditorGUI.GetPropertyHeight(item, GUIContent.none, true);

								return Mathf.Max(default_height, compute_height) + XEditorInspector.SPACE;
							}
						}
						else
						{
