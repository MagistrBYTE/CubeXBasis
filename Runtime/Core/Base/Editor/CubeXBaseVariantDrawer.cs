...
//---------------------------------------------------------------------------------------------------------------------
/** \file CubeXBaseVariantDrawer.cs
...
public class CubeXBaseVariantDrawer : PropertyDrawer
{
	...
	{
		Single height = XEditorInspector.CONTROL_HEIGHT;
		if (variant.ValueType == TValueType.Rect)
		{
			if (EditorGUIUtility.wideMode)
			{
				height = XEditorInspector.CONTROL_HEIGHT_SPACE * 2;
			}
			else
			{
				height = XEditorInspector.CONTROL_HEIGHT * 3 + XEditorInspector.SPACE * 2;
			}
		}
		else
		{
				...
				variant.ValueType == TValueType.Vector4D)
			{
				...
				{
					height = (XEditorInspector.CONTROL_HEIGHT * 2 + XEditorInspector.SPACE);
				}
			}
		}
		...
		Boolean view_button, Boolean color_select)
	{
		...
		if (view_button)
		{
			...
			rect_field.width = position.width - EditorGUIUtility.labelWidth - XEditorInspector.BUTTON_MINI_WIDTH - 1;
			...
			case TValueType.Real:
				{
					...
					EditorGUI.BeginChangeCheck();
					{
						...
						variant.RealValue = EditorGUI.FloatField(rect_field, variant.RealValue);
						if (color_select) GUI.backgroundColor = Color.white;
					}
					if (EditorGUI.EndChangeCheck())
					{
			...
			case TValueType.Rect:
				{
					...
					EditorGUI.BeginChangeCheck();
					{
						variant.RectValue = EditorGUI.RectField(rect_field, variant.RectValue);
					}
					if (EditorGUI.EndChangeCheck())
					{
						changed_value = true;
