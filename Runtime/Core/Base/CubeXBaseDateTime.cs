...
//---------------------------------------------------------------------------------------------------------------------
/** \file CubeXBaseDateTime.cs
*		Работа с датой/временем.
*		Реализация общий типов и структур данных для работы с датой/временем.
*/
//---------------------------------------------------------------------------------------------------------------------
// Версия: 1.0.0.0
...
namespace CubeX
{
	namespace Core
	{
		...
		/// </summary>
		//-------------------------------------------------------------------------------------------------------------
		public interface ICubeXDateTimeable : ICloneable
		{
			/// <summary>
			/// Дата
			/// </summary>
		...
		//-------------------------------------------------------------------------------------------------------------
		/// <summary>
		/// Коллекция содержащая объекты реализующие интерфейс <see cref="ICubeXDateTimeable"/> с определённым временным интервалом
		/// </summary>
		/// <remarks>
		...
		//-------------------------------------------------------------------------------------------------------------
		public class ListTimeInterval<TItemTimeable> : ListArray<TItemTimeable> where TItemTimeable : ICubeXDateTimeable
		{
			#region ======================================= ДАННЫЕ ====================================================
			// Основные параметры
			...
			/// </summary>
			/// <remarks>
			/// Временной интервал обозначает что свойство <see cref="ICubeXDateTimeable.Date"/> элементов списка 
					...
					case TTimeInterval.Minutely:
						{
							...
							{
								current = current + TimeSpan.FromMinutes(1);
					...
					case TTimeInterval.Hourly:
						{
							while (current < end_data)
							{
								current = current + TimeSpan.FromHours(1);
