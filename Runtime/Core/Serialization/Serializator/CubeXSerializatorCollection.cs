...
//---------------------------------------------------------------------------------------------------------------------
/** \file CubeXSerializatorCollection.cs
...
namespace CubeX
{
	namespace Core
	{
		...
		//-------------------------------------------------------------------------------------------------------------
		/// <summary>
		/// Статический класс реализующий сериализацию коллекций в различные форматы
		/// </summary>
		//-------------------------------------------------------------------------------------------------------------
		public static class XSerializatorCollection
		{
			...
			//---------------------------------------------------------------------------------------------------------
			public static void WriteCollectionToXml(XmlWriter writer, Type element_type, IList collection, String element_name)
			{
				if (collection != null)
				{
						...
						case TSerializeDataType.Primitive:
							{
								for (Int32 i = 0; i < collection.Count; i++)
								{
									...
									XSerializatorPrimitive.WriteToAttribute(writer, element_type, collection[i], XSerializationDispatcher.XML_NAME_ATTRIBUTE_VALUE);
						...
						case TSerializeDataType.Struct:
							{
								...
								else
								{
									for (Int32 i = 0; i < collection.Count; i++)
									{
										XSerializatorObject.WriteInstanceToXml(writer, serialize_data, collection[i]);
									}
								}
							}
							break;
						case TSerializeDataType.Class:
							{
								...
								for (Int32 i = 0; i < collection.Count; i++)
								{
									XSerializatorObject.WriteInstanceToXml(writer, collection[i], null);
								}
							}
							break;
#if UNITY_2017_1_OR_NEWER
						...
						case TSerializeDataType.UnityUserComponent:
							{
								...
								{
									writer.WriteStartElement(element_type.Name);
									XSerializatorUnity.WriteReferenceComponentToXml(writer, collection[i] as UnityEngine.Component);
									writer.WriteEndElement();
								}
							}
							break;
						case TSerializeDataType.UnityGameObject:
							{
								...
								{
									writer.WriteStartElement(element_type.Name);
									XSerializatorUnity.WriteReferenceGameObjectToXml(writer, collection[i] as UnityEngine.GameObject);
									writer.WriteEndElement();
								}
							}
							break;
						...
						case TSerializeDataType.UnityUserResource:
							{
								...
								{
									writer.WriteStartElement(element_type.Name);
									XSerializatorUnity.WriteReferenceResourceToXml(writer, collection[i] as UnityEngine.Object);
									writer.WriteEndElement();
								}
							}
							break;
			...
			//---------------------------------------------------------------------------------------------------------
			public static IList ReadCollectionFromXml(XmlReader reader, IList collection, String element_name, Int32 count)
			{
					...
					while (reader_subtree.Read())
					{
						...
						if (reader_subtree.NodeType == XmlNodeType.Element && reader_subtree.Name != element_name)
						{
							...
							if (collection.IsFixedSize)
							{
								...
								{
									collection.SetAt(index, XSerializationDispatcher.ReadDataFromXml(reader_subtree, null, index));
									index++;
								}
							}
							else
