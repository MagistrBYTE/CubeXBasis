...
namespace CubeX
{
	namespace Core
	{
		...
		public interface ICubeXCollectionModel : ICubeXModel
		{
			...
			/// <returns>Истина если объетк или элемент проходят условия проверки предикатом</returns>
			//--------------------------------------------------------------------------------------------------------
			Boolean FiltredModel(Predicate<ICubeXModel> match);

			//---------------------------------------------------------------------------------------------------------
			/// <summary>
			/// Посещение элементов указанным посетителем
			/// </summary>
			/// <param name="on_visitor">Делегат посетителя</param>
			//---------------------------------------------------------------------------------------------------------
			void VisitModel(Action<ICubeXModel> on_visitor);
			#endregion
		}
